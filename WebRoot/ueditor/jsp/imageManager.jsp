...
<% 
    //仅做示例用，请自行修改
	String path = "upload\\ueImage";
	String imgStr ="";
//	String realpath = getRealPath(request,path)+"\\"+path;
    String realpath = getRealPath(request,path).replace("ueditor\\jsp","")+path;
	List<File> files = getFiles(realpath,new ArrayList());

	for(File file :files ){
//		imgStr+=file.getPath().replace(getRealPath(request,path),"")+"ue_separate_ue";
		imgStr+= "../../"+file.getPath().replace(getRealPath(request,path).replace("ueditor\\jsp",""),"")+"ue_separate_ue";
	}
	if(imgStr!=""){
