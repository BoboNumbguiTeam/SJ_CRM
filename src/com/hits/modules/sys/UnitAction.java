...
public class UnitAction extends BaseAction {
	...
	public String addSave(@Param("..") Sys_unit unit) {
		String id = daoCtl.getSubMenuId(dao,"sys_unit", "id",
				StringUtil.null2String(unit.getId()));
		unit.setId(id);
	...
	@At
	@Ok("raw")
	public String telunit(HttpSession session, HttpServletRequest req,@Param("telephone") String phone) {
		String sql = "select * from sys_unit where id LIKE '0002____' and telephone is not null ";
		if(phone!=null&&!phone.equals("")){
			sql+=" and telephone = '"+phone+"'";
		}
		sql+=" order by id asc";
		List<Sys_unit> list = daoCtl.list(dao,Sys_unit.class,Sqls.create(sql));
		Gson gson=new Gson();
